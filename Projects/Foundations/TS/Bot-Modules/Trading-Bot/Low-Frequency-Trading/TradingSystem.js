exports.newFoundationsBotModulesTradingSystem = function (processIndex) {
    ...
    */
    ...
    let tradingStagesModuleObject =
      TS.projects.foundations.botModules.tradingStages.newFoundationsBotModulesTradingStages(
        processIndex
      );

    let taskParameters = {
      market:
        TS.projects.foundations.globals.taskConstants.TASK_NODE.parentNode
          .parentNode.parentNode.referenceParent.baseAsset.referenceParent
          .config.codeName +
        "/" +
        TS.projects.foundations.globals.taskConstants.TASK_NODE.parentNode
          .parentNode.parentNode.referenceParent.quotedAsset.referenceParent
          .config.codeName,
    };
    return thisObject;
    ...
    function initialize() {
      tradingSystem =
        TS.projects.foundations.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
          processIndex
        ).SIMULATION_STATE.tradingSystem;
      tradingEngine =
        TS.projects.foundations.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
          processIndex
        ).SIMULATION_STATE.tradingEngine;
      sessionParameters =
        TS.projects.foundations.globals.processConstants.CONSTANTS_BY_PROCESS_INDEX_MAP.get(
          processIndex
        ).SESSION_NODE.tradingParameters;

      // NICO
      var fs = require("fs");
      let customPath =
        __dirname + "/../../../../../../Custom/Libs/tradingHelper.js";
      helper = fs.readFileSync(customPath, "utf8");
      ...
      tradingSystem.evalConditions = function (
        startingNode,
      ...
      tradingSystem.evalFormulas = function (
        startingNode,
        descendentOfNodeType
      ) {
      ...
      tradingSystem.evalUserCode = function (
        startingNode,
        descendentOfNodeType
      ) {
      ...
      function isItInside(elementWithTimestamp, elementWithBeginEnd) {
        /*
            ...
            */
        if (
          elementWithTimestamp.timestamp >= elementWithBeginEnd.begin &&
          elementWithTimestamp.timestamp <= elementWithBeginEnd.end
        ) {
          return true;
        } else {
          return false;
    ...
    function buildDynamicIndicators() {
      if (tradingSystem.dynamicIndicators !== undefined) {
        ...
        tradingSystem.evalFormulas(
          tradingSystem.dynamicIndicators,
          "Indicator Function"
        );

        for (
          let i = 0;
          i < tradingSystem.dynamicIndicators.indicatorFunctions.length;
          i++
        ) {
          let indicatorFunction =
            tradingSystem.dynamicIndicators.indicatorFunctions[i];
          if (indicatorFunction.formula === undefined) {
            return;
          }
          if (indicatorFunction.config.codeName === undefined) {
            return;
          }
          dynamicIndicators[indicatorFunction.config.codeName] =
            tradingSystem.formulas.get(indicatorFunction.formula.id);
        }
      }
    }
      ...
      try {
        ...
        if (err.stack !== undefined) {
          TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
            ...
            MODULE_NAME,
            "[ERROR] runExecution -> err = " + err.stack
          );
        }
      }
    }
    ...
    function evalNode(node, evaluating, descendentOfNodeType, isDescendent) {
      ...
      if (
        (node.type === "Formula" || node.type === "Announcement Formula") &&
        evaluating === "Formulas"
      ) {
      ...
      let schemaDocument =
        TS.projects.foundations.globals.taskConstants.APP_SCHEMA_MAP.get(
          node.project + "-" + node.type
        );
      ...
      if (schemaDocument.childrenNodesProperties !== undefined) {
        ...
        for (
          let i = 0;
          i < schemaDocument.childrenNodesProperties.length;
          i++
        ) {
    ...
    function evalCondition(node) {
      ...
      if (node.javascriptCode !== undefined) {
        if (node.javascriptCode.code !== undefined) {
          code = helper + node.javascriptCode.code;
      ...
      try {
        TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
          ...
          MODULE_NAME,
          "[INFO] evalCondition -> " + node.name + " -> code = " + code
        );
        value = eval(code);
      } catch (err) {
        /*
      ...
      TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
        ...
        MODULE_NAME,
        "[INFO] evalCondition -> value = " + value
      );
      TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
        processIndex
      ).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(
        MODULE_NAME,
        "[INFO] evalCondition -> errorMessage = " + errorMessage
      );
    }
